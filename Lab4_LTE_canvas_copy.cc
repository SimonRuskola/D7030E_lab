...
main (int argc, char *argv[])
{
  ...
  //int distance = 800;
  for(int distance = 200; distance <= 1300; distance += 100){
...
std::string dataRate = "50.0Mbps";
  ...
  if (outputpath!="")
  {
    Config::SetDefault ("ns3::RadioBearerStatsCalculator::DlRlcOutputFilename", StringValue (outputpath+"/DlRlcStats"+dataRate+distanceString+".txt"));
    Config::SetDefault ("ns3::RadioBearerStatsCalculator::UlRlcOutputFilename", StringValue (outputpath+"/UlRlcStats"+dataRate+distanceString+".txt"));
    Config::SetDefault ("ns3::RadioBearerStatsCalculator::DlPdcpOutputFilename", StringValue (outputpath+"/DlPdcpStats"+dataRate+distanceString+".txt"));
    Config::SetDefault ("ns3::RadioBearerStatsCalculator::UlPdcpOutputFilename", StringValue (outputpath+"/UlPdcpStats."+dataRate+distanceString+".txt"));
    Config::SetDefault ("ns3::MacStatsCalculator::DlOutputFilename", StringValue (outputpath+"/DlMacStats"+dataRate+distanceString+".txt"));
    Config::SetDefault ("ns3::MacStatsCalculator::UlOutputFilename", StringValue (outputpath+"/UlMacStats"+dataRate+distanceString+".txt")); 
  }
  ...
  MM -> SetPosition(Vector3D(distance,0,1)); // distance between UE and eNodeB is 5 metres
...
// Install and start applications on UE and remote host
 ...
 uint16_t dlPort = 1000;
  ...
  //p2ph.EnablePcap ("LTE_", internetDevices.Get (1), true);
